...
- [ ] **Short description** (1-2 sentences, for listing):
  > "Aroha AI is an Android health companion for seniors managing multiple medications. It combines a conversational AI with persistent memory, camera-based medication capture, ABDM health-record screenshot import, a daily schedule with reminders, and one-tap doctor-visit summaries."
- [ ] **Detailed description** (100-200 words, for judging):
  > Draft:
  > "Aroha AI tackles two real problems for seniors: medication mismanagement and scattered health records. The app pairs an AI chat with persistent memory and a simple, big-text daily schedule. Its standout is a single Gemini Vision pipeline serving two jobs: photograph a pill strip and Aroha reads the medication, dosage, and timing and schedules it with a reminder; or upload a screenshot of your ABDM/ABHA record and Aroha auto-fills your entire health profile — import, not a months-long integration. Users log symptom photos with timestamps, and one tap generates a doctor-visit summary of symptoms, adherence, and questions to ask. Built with React Native (Expo, Android-first) and the Gemini API. Data stays on-device; the Gemini key lives only in a Cloudflare Worker proxy — never in the app or repo. Aroha is deliberately positioned as an advocate that prepares patients for their doctor — it never diagnoses or overrides a physician. Roadmap: accounts + cloud sync, voice, native ABDM linking, prescription-review advocacy, and a caregiver dashboard."
